...
public class CommonSmileDaoImpl extends HBaseSupport implements BaseDao<CommonSmile>
{
	private static String TAB_NAME = "common_smile";

	@Override
	public void put(final CommonSmile instance)
	{
		ht.execute(TAB_NAME, new TableCallback<Result>()
				{
					...
					{
						Put p = new Put(Bytes.toBytes(instance.getSid()));
						p.add(Bytes.toBytes("info"), Bytes.toBytes("description")
								, Bytes.toBytes(instance.getDescription()));
						p.add(Bytes.toBytes("info"), Bytes.toBytes("imgurl")
								, Bytes.toBytes(instance.getImgurl()));
						p.add(Bytes.toBytes("info"), Bytes.toBytes("order")
								, Bytes.toBytes(String.valueOf(instance.getOrder())));
						htable.put(p);
						return null;
					}
