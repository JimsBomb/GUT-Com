...
public class CommonSyslogDaoImpl extends HBaseSupport implements BaseDao<CommonSyslog>
{
	private static String TAB_NAME = "common_syslog";

	@Override
	public void put(final CommonSyslog instance)
	{
		ht.execute(TAB_NAME, new TableCallback<Result>()
				{
					...
					{
						Put p = new Put(Bytes.toBytes(instance.getLid()));
						p.add(Bytes.toBytes("info"), Bytes.toBytes("detail")
								, Bytes.toBytes(instance.getDetail()));
						p.add(Bytes.toBytes("info"), Bytes.toBytes("ip")
						...
						p.add(Bytes.toBytes("info"), Bytes.toBytes("type")
								, Bytes.toBytes(String.valueOf(instance.getType())));
						htable.put(p);
						return null;
					}
