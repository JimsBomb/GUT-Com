...
public class CommonSysconfDaoImpl extends HBaseSupport implements BaseDao<CommonSysconf>
{
	private static String TAB_NAME = "common_sysconf";

	@Override
	public void put(final CommonSysconf instance)
	{
		ht.execute(TAB_NAME, new TableCallback<Result>()
				{
					...
					{
						Put p = new Put(Bytes.toBytes(instance.getConfname()));
						p.add(Bytes.toBytes("info"), Bytes.toBytes("confvalue")
								, Bytes.toBytes(instance.getConfvalue()));
						htable.put(p);
						return null;
					}
