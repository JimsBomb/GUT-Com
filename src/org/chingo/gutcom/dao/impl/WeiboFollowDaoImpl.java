...
public class WeiboFollowDaoImpl extends HBaseSupport implements BaseDao<WeiboFollow>
{
	private static String TAB_NAME = "weibo_follow";

	@Override
	public void put(final WeiboFollow instance)
	{
		ht.execute(TAB_NAME, new TableCallback<Result>()
				{
					...
					public Result doInTable(HTableInterface htable) throws Throwable
					{
						...
						p.add(Bytes.toBytes("info"), Bytes.toBytes("followid")
								, Bytes.toBytes(instance.getFollowId()));
						p.add(Bytes.toBytes("info"), Bytes.toBytes("groupname")
								, Bytes.toBytes(instance.getGroupName()));
						p.add(Bytes.toBytes("info"), Bytes.toBytes("remark")
								, Bytes.toBytes(instance.getRemark()));
						htable.put(p);
						return null;
					}
